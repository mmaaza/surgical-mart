...
const PaymentStep = ({ onBack, onComplete, orderSummary }) => {
  ...
  const validateFile = useCallback((file) => {
    const validation = paymentService.validatePaymentFile(file);
  ...
  const formErrors = useMemo(() => {
    ...
    // Only validate screenshot requirement if user has explicitly selected pay-now AND tried to submit
    if (paymentMethod === PAYMENT_METHODS.PAY_NOW && !screenshotFile) {
      // Don't show error immediately, only after user interaction
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      // Validate payment data using the service
      const paymentValidation = paymentService.validatePaymentMethod(paymentData);
      if (!paymentValidation.isValid) {
        throw new Error(paymentValidation.errors[0]);
      }
      ...
      try {
        await notificationService.createNotification({
          ...
          message: `A new order has been placed with ${paymentData.paymentMethod === 'pay-now' ? 'online payment' : 'cash on delivery'}`,
          data: {
            orderNumber: orderSummary?.orderNumber || 'N/A',
            userName: orderSummary?.userName || 'Customer',
  ...
  }, [paymentMethod, screenshotFile, isFormValid, onComplete, orderSummary]);
