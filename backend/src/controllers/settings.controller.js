...
exports.getSocialSettings = async (req, res) => {
  try {
    let settings = await Settings.findOne({ key: 'social' });
...
// @route   POST /api/admin/settings/social
// @access  Private/Admin
exports.updateSocialSettings = async (req, res) => {
  try {
    const { 
      ...
      shareMessage,
      shareButtonsEnabled,
      followButtonsEnabled
    } = req.body;

    let socialSettings = await Settings.findOne({ key: 'social' });
    
    if (!socialSettings) {
      socialSettings = new Settings({ 
        key: 'social',
        updatedBy: req.user._id
      });
    }
    ...
    socialSettings.data = {
      ...
      shareButtonsEnabled: shareButtonsEnabled !== undefined ? Boolean(shareButtonsEnabled) : true,
      followButtonsEnabled: followButtonsEnabled !== undefined ? Boolean(followButtonsEnabled) : true
    };
    
    socialSettings.updatedBy = req.user._id;
    await socialSettings.save();

    res.status(200).json({
      success: true,
      data: socialSettings.data
    });
  } catch (error) {
...
// @desc    Get public social settings
// @route   GET /api/settings/social
// @access  Public
exports.getPublicSocialSettings = async (req, res) => {
  try {
    const settings = await Settings.findOne({ key: 'social' });
    ...
    console.error('Error fetching public social settings:', error);
    res.status(500).json({
      success: false,
      error: 'Server Error'
    });
